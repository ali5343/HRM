<x-layout>
    ...
    <script>
        document.getElementById('type').addEventListener('change', function () {
            ...
            if (this.value === 'leave') {
                startTime.type = 'date'; // Show date picker for leave
                endTime.type = 'date';

                // Set default start and end times for each selected day
                startTime.addEventListener('change', () => {
                    const selectedDate = startTime.value;
                    startTime.type = 'datetime-local';
                    startTime.value = selectedDate + 'T09:00';
                    endTime.type = 'datetime-local';
                    endTime.value = selectedDate + 'T17:00';
                });

            } else {
                startTime.type = 'datetime-local'; // Restore datetime picker for other options
                endTime.type = 'datetime-local';
                startTime.value = '';
                endTime.value = '';
            }
